...
We first built a spectroscopic reference sample within the DP2 footprint by concatenating a large compilation of $\sim$43 million redshifts from 38 reference redshift catalogs using the \texttt{Combine Redshift Catalogs} pipeline\footnote{\url{https://docs.linea.org.br/en/sci-platforms/pz_server_crc.html}} from the LIneA PZ Server\footnote{\url{https://pzserver.linea.org.br}}. The pipeline uses \textsc{LSDB} to perform an efficient all-to-all cross-matching with the reference catalogs to identify multiple measurements of the same objects and resolve these duplicates according to data quality criteria based on previously homogenized quality flags, prioritizing spectroscopic data over grism measurements and grism measurements over high-quality \photoz estimates from multi-band imaging (Gschwend et al., in preparation). As a result, we produced two redshift reference catalogs, one as a compilation with redshifts of $\sim$6.6 million unique galaxies from 37 reference surveys, and DESI DR1, separately, with homogenized flags.     

We then use the \texttt{Training Set Maker} pipeline\footnote{\url{https://docs.linea.org.br/en/sci-platforms/pz_server_tsm.html}}, which also uses \textsc{LSDB} as its back end, to cross-match the compilation and DESI DR1 against the DP2 object catalog, separately, retaining the nearest photometric counterpart within a fixed matching radius of 1.0 arcsec. The resulting reference sets span both the deep drilling fields, including COSMOS, ELAIS, ECDFS, and EDFS, for which many high-quality redshifts exist, and several wide-field spectroscopic surveys, most notably DESI and, for the compilation, SDSS. In Table~\ref{tab:dp2_refcat}, we report the number of cross-matched galaxies contributed by each survey, listing all surveys with more than 100 objects and grouping the remainder into a single ``Other'' row.  The cross-matched data are strongly dominated by the wide-field DESI, SDSS spectroscopic samples and the COSMOS deep-field compilations (counted as one reference source). In Fig.~\ref{fig:train_radec}, we show the right-ascension--declination distribution of the training galaxies, color-coded by their originating survey.
...
\textcolor{green}{from pzserver import PzServer}
\textcolor{green}{pz_server = PzServer(token="<paste your access token here>")}
\end{Verbatim}
...
\begin{Verbatim}[commandchars=\\\{\}]
\textcolor{green}{pz_server.display_product_metadata("<product_id>")}
\textcolor{green}{pz_server.download_product("<product_id>", save_in=".")} 
\end{Verbatim}
...
\begin{Verbatim}[commandchars=\\\{\}]
\textcolor{green}{z_catalog = pz_server.get_product("318_ref_z_clean_no_desi_lss").to_pandas()}
\end{Verbatim}
...
\hline
\end{tabular}
\caption{Redshift reference catalogs described in Section~\ref{sec:data:reference}, removed duplicates and objects with bad quality (\texttt{homogenized\_flag} $< 3$). The links to all the original reference catalogs are available at the PZ Server Data documentation page.}
